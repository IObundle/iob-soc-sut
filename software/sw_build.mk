...
# FIRMWARE SOURCES
FW_SRC+=src/iob_soc_sut_firmware.S
FW_SRC+=src/iob_soc_sut_firmware.c
FW_SRC+=src/printf.c
...
# BOOTLOADER SOURCES
BOOT_SRC+=src/iob_soc_sut_boot.S
BOOT_SRC+=src/iob_soc_sut_boot.c
BOOT_SRC+=$(filter-out %_emul.c, $(wildcard src/iob*uart*.c))
BOOT_SRC+=$(filter-out %_emul.c, $(wildcard src/iob*cache*.c))

build_software:
	make iob_soc_sut_firmware.elf INCLUDES="$(INCLUDES)" LFLAGS="$(LFLAGS) -Wl,-Map,iob_soc_sut_firmware.map" SRC="$(FW_SRC)"
	make iob_soc_sut_boot.elf INCLUDES="$(INCLUDES)" LFLAGS="$(LFLAGS) -Wl,-Map,iob_soc_sut_boot.map" SRC="$(BOOT_SRC)"
